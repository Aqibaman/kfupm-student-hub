  ...
  return (
    <div className="space-y-6">
      <PageIntro
        ...
        description="Check each washer and dryer state, including available, reserved, in use, and out-of-service status with the next free time."
        backHref="/laundry"
      />
        ...
        {laundryMachines.map((machine) => (
          <SoftCard key={machine.id}>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Next free time: {machine.nextFreeTime}
            </p>

            <div className="mt-5 flex flex-wrap gap-3">
              <ActionLink
                href={machine.type === "Washer" ? "/laundry/washer" : "/laundry/dryer"}
                label={`Book ${machine.type}`}
                variant="primary"
              />
            </div>
          </SoftCard>
