...
export default function LaundryHistoryPage() {
  const bookings = getLaundryBookings().filter((booking) => booking.status === "Completed" || booking.status === "Missed" || booking.status === "Cancelled");

  return (
    <div className="space-y-6">
      <PageIntro title="Laundry History" description="Review successful, missed, and cancelled reservations with clear labels for usage result and credit impact." backHref="/laundry" />
          ...
          <SoftCard key={booking.id}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="text-right">
                ...
                <p className="mt-1 text-sm text-slate-500">Credits {booking.creditsDelta >= 0 ? `+${booking.creditsDelta}` : booking.creditsDelta}</p>
              </div>
            </div>
          </SoftCard>
        ))}
